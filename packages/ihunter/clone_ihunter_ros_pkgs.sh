#!/usr/bin/env bash
#
# SUPERSEDED on 2026-09-09 by ihunter.repos.
#
# This script drifted, which is what scripts like it do: by the time it was
# retired it was missing geo_tuner, mav_navigator_ros, ihunter_system and
# interception_guidance -- four of the packages the vehicle actually flies --
# and it pinned nothing, so two runs a month apart gave different code.
#
# Use the manifest instead. It lists every package with an exact commit:
#
#     cd ~/ihunter_shared_volume && vcs import < .../packages/ihunter/ihunter.repos
#
# or let bootstrap.sh do it as part of setting up a fresh board.
#
set -e
cat >&2 <<'MSG'
clone_ihunter_ros_pkgs.sh is superseded by ihunter.repos.

  cd ~/ihunter_shared_volume
  vcs import < <jetson-containers>/packages/ihunter/ihunter.repos

Every package there is pinned to a commit, which this script never was.
The original is kept below this line for reference only.
MSG
exit 1

# ---------------------------------------------------------------------------
# ORIGINAL, for reference. Do not run.
# ---------------------------------------------------------------------------
# #!/bin/bash
